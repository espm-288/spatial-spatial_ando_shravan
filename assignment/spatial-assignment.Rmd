...
```{r}
d = st_dimensions(birm_ndvi)
e <- ext(6.15, 6.3, 49.7, 49.8)
e <- ext(-86.756777, -86.75692, -86.762018, -86.762268)

birm_cropped <- crop(birm_terra, e)
plot(birm_cropped)

tmap_mode("view")
holc_mask <- mask(birm_ndvi, birm_terra)
tm_shape(holc_mask) + tm_raster(style= "cont")


tm_shape(holc_mask, bbox = st_bbox(birm)) + tm_raster() +
   tm_shape(birm) + tm_polygons("holc_grade", alpha=0.1) + tm_borders(col="holc_grade")
