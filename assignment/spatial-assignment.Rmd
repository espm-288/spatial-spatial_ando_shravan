...
### Function to get the distribution of NDVI across different holc grade (and redlined areas)
```{r}
...
```{r}
#Getting the aggregated hold grade for all cities
...
### Show cropped NDVI to polygons of interest
```{r, eval=FALSE}
#Test code : delete
tmap_mode("view")
holc_mask <- terra::mask(x=birm_ndvi, mask=birm_terra)
tm_shape(holc_mask, bbox = st_bbox(birm)) + tm_raster()


tm_shape(holc_mask, bbox = st_bbox(birm)) + tm_raster(palette = terrain.colors(4)) +
   tm_shape(birm, is.master = TRUE) + tm_polygons("holc_grade", alpha=1) #+ tm_borders(col="holc_grade")

tm_shape(birm_ndvi, bbox = st_bbox(birm)) + tm_raster(palette = terrain.colors(5)) +
   tm_shape(birm) + tm_polygons("holc_grade", alpha=.1) #+ tm_borders(col="holc_grade")

holc_mask_inv <- 1 - holc_mask
holc_mask_inv

tm_shape(holc_mask_inv, bbox = st_bbox(birm)) + tm_raster(palette = -terrain.colors(5)) +
   tm_shape(birm) + tm_polygons("holc_grade", alpha=.1,border.col='red') #+ tm_fill(col="holc_grade") 

```

## TODO: Convert this into a function and call for each city
...
tmap_mode("plot")
birm_A <- birm[birm$holc_grade == "A",]
birm_B <- birm[birm$holc_grade == "B",]
birm_C <- birm[birm$holc_grade == "C",]
birm_D <- birm[birm$holc_grade == "D",]


tm_shape(holc_mask, bbox = st_bbox(birm)) + tm_raster(palette = get_brewer_pal(palette="YlGn", n=5, plot=FALSE), title = "NDVI") +
   tm_shape(birm_A) + tm_polygons(x="holc_grade", title="A",alpha=0,border.col='green', title="A")+
   tm_shape(birm_B) + tm_polygons(x="holc_grade", title="B",alpha=0,border.col='blue')+
   tm_shape(birm_C) + tm_polygons(x="holc_grade", title="C",alpha=0,border.col='orange')+
   tm_shape(birm_D) + tm_polygons(x="holc_grade", title="D",alpha=0,border.col='red') + 
   tm_add_legend('title', 
               	...
               	title="Redlining") +
   tm_layout(main.title = "Birmingham", title.size = 1, 
